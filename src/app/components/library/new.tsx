    ...
    const onSubmitHandle = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            reader.onload = async () => {
              ...
              const data = { cover: base64Data, name, description, allowData, allowUsers }
              ...
              if(respData.message==="success"){
                alert('Éxito la categoría se registró correctamente')
              } else if (respData.message === "exists"){
                alert('Error: la categoria ya existe')
                setErrorMessageName('Error: la categoria ya existe, ingrese otro nombre porfavor')
              } else {
                ...
                <div className="relative flex items-center">
                  <div className="w-full z-10">                    
                    ...
                    <form className="mt-8 space-y-6" action="#" method="POST" onSubmit={onSubmitHandle} >
                      ...
                      {selectedCover && (
                        <img src={URL.createObjectURL(selectedCover)} alt="Preview" style={{ maxWidth: '100%', borderRadius: 10 }} />
                      )}
                      {errorMessageCover && 
                      ...
                      <div className="relative">                        
                        ...
                        <input onChange={onChangeName} className="w-full text-base py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" style={{color: 'black'}} type="" placeholder="Matemáticas, Ciencias, Deportes" />
                        {errorMessageName && 
                          <div style={{maxWidth: 500}}>
                      ...
                      <div className="mt-8 content-center">
                        ...
                        <input onChange={onChangeDescription} className="w-full content-center text-base py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" style={{color: 'black'}} type="" placeholder="Descripción de la categoría" />
                      ...
                      <div>
                        <button type="submit" disabled={loading} className="w-full flex justify-center bg-indigo-500 text-gray-100 p-4  rounded-full tracking-wide font-semibold  focus:outline-none focus:shadow-outline hover:bg-indigo-600 shadow-lg cursor-pointer transition ease-in duration-300">
                            {!loading?'Crear categoría':'Creando...'}
                        </button>
                      </div>
...
export default NewLibraryComponent